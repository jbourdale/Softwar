...
int check_valid_leftfwd(t_thread_options *options, t_player *player)
{
...
int command_left_forward(zframe_t *identity, char *data, t_thread_options *options, zsock_t *router)
{
	(void) data;
	t_player *player;
	int *pos;
	...
	if (check_valid_leftfwd(options, player) == 1)
	{
		// looking left
		...
		{
			log_trace("[COMMAND_LEFTFWD] %s moving left forward to (%d, %d)", player_str, pos[0], pos[1]);
			player->x = pos[0];
			player->y = pos[1];
			player->actionpoint -= 1;
		...
		{
			log_warn("[COMMAND_LEFTFWD] %s can't move left forward to (%d, %d)", player_str, pos[0], pos[1]);
			// cancel looking left
		...
		log_warn("[COMMAND_LEFTFWD] %s can't move left forward now", player_str);
